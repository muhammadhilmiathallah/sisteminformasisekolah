<template>
  <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-md px-8 py-10 space-y-8">
    <h2 class="text-3xl font-bold text-gray-800 flex items-center gap-3"><i class="pi pi-print text-purple-600 text-2xl"></i> Cetak Raport Semua Siswa</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <!-- <div class="flex flex-col">
        <label class="text-sm font-semibold text-gray-700 mb-2">Kelas (otomatis dari guru login)</label>
        <InputText v-model="kelasLogin.nama_kelas" class="w-full" disabled />
      </div> -->

      <div class="flex flex-col">
        <label class="text-sm font-semibold text-gray-700 mb-2">Pilih Tahun Pelajaran</label>
        <Dropdown v-model="selectedTahun" :options="tahunPelajaranList" optionLabel="tahun_pelajaran" optionValue="id" placeholder="Pilih Tahun Pelajaran" class="w-full" />
      </div>
    </div>

    <div class="pt-6 flex justify-end">
      <Button label="Cetak Raport Semua" icon="pi pi-print" @click="cetakRaport" :disabled="!kelasLogin.id || !selectedTahun" class="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-md border-none shadow-md" />
    </div>
  </div>
</template>
