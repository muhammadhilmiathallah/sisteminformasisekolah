  ...
  <div class="card">
    ...
    <div class="p-fluid p-formgrid p-grid">
      <div class="p-field p-col-12 p-md-6">
        ...
        <Dropdown v-model="selectedTahunPelajaranId" :options="tahunPelajaranList" optionLabel="tahun_pelajaran" optionValue="id" placeholder="Pilih Tahun Pelajaran" @change="fetchNilaiSiswa" />
      </div>
      <div class="p-field p-col-12 p-md-6">
        <label for="siswa">Pilih Siswa</label>
        <Dropdown v-model="selectedSiswaId" :options="siswaList" optionLabel="nama_lengkap_siswa" optionValue="id" placeholder="Pilih Siswa" @change="fetchNilaiSiswa" />
      </div>
    </div>

    <div v-if="nilaiList.length" class="mt-4">
      <Button label="Cetak Raport" icon="pi pi-print" class="p-button-success mb-3" @click="cetakRaport" />
      <DataTable :value="nilaiList" responsiveLayout="scroll">
        <Column field="nama_mapel" header="Mata Pelajaran" />
        <Column field="nilai_tugas" header="UH" />
...
const fetchNilaiSiswa = async () => {
  ...
  try {
    const res = await axios.get(`http://127.0.0.1:8000/api/nilai/siswa/${selectedSiswaId.value}/tahun/${selectedTahunPelajaranId.value}`);
...
const getStatus = (grade) => {
  if (['A','B','C'].includes(grade)) return 'Tuntas'
  if (['D'].includes(grade)) return 'Remedial'
  return 'Tidak Lulus'
}

const getStatusClass = (grade) => {
  if (['A','B','C'].includes(grade)) return 'text-success'
  if (['D'].includes(grade)) return 'text-warning'
  return 'text-danger'
}
</script>
