    ...
    <table>
        ...
        <tbody>
            @foreach ($data as $index => $siswa)
                <tr>
                    ...
                    <td style="background-color: purple; color: white;">
                        {{ number_format($siswa['jumlah'] ?? 0, 2) }}
                    </td>
                    <td style="background-color: maroon; color: white;">
                        {{ number_format($siswa['rata_rata'] ?? 0, 2) }}
                    </td>
                    <td style="background-color: rgb(66, 66, 201); color: white;">
                        {{ $siswa['rank'] }}
                    </td>
                </tr>
            @endforeach
            ...
            <tr class="highlight">
                ...
                @endforeach
                <td>{{ number_format(collect($data)->pluck('jumlah')->filter()->avg() ?? 0, 2) }}</td>
                <td>{{ number_format(collect($data)->pluck('rata_rata')->filter()->avg() ?? 0, 2) }}</td>
                <td></td>
            </tr>
        </tbody>
    </table>
